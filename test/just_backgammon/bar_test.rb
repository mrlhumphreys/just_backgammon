...
describe JustBackgammon::Bar do
  ...
  describe 'pieces owned by player' do
    let(:bar) { JustBackgammon::Bar.new(pieces: [{id: 1, player_number: 1}, {id: 2, player_number: 1}]) }
  ...
  describe 'pop' do
    let(:bar) { JustBackgammon::Bar.new(pieces: [{id: 1, player_number: 1}, {id: 2, player_number: 2}]) }

    it 'must remove a piece from the bar for that player' do
      bar.pop(1)
      assert_equal 1, bar.pieces.size
      assert_equal 2, bar.pieces.first.player_number
    end
  ...
  describe 'push' do
    ...
    let(:bar) { JustBackgammon::Bar.new(pieces: [{id: 2, player_number: 1}, {id: 3, player_number: 2}]) }
